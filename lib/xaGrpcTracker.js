...
class Tracker {
   ...
   */
  constructor(ctx, call) {
    assert(call && call.request, '[grpc tracker] - no grpc request detected');
    assert(call && call.request.trace_id && call.request.span_id, '[grpc tracker] - trace id and parent id are required');

    const { trace_id, span_id: parent_id } = call.request;
    this.ctx = ctx;
