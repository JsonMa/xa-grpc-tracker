...
class Tracker {
   ...
   * @param {Object} spanName    - span name
   * @param {Object} call        - grpc call
   */
  constructor(ctx, spanName, call) {
    assert(spanName, '[grpc tracker] - span name is required');
    ...
    this.trackerParams = {
      ...
      span_name: this.trackerConfig.span_name || ctx.app.config.name.replace(/-/gi, '_'),
      DLN,
    };
    this.endPoint = {
      addrs: {
